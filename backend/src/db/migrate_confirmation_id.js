...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://evans:password@localhost:5432/graduation_db'
});
...
async function migrateConfirmationId() {
  ...
  try {
    ...
    if (checkColumn.rows.length > 0 && checkColumn.rows[0].data_type === 'character varying') {
      console.log('confirmation_id column is already VARCHAR. Migration not needed.');
      return;
    }
    ...
    await client.query('ALTER TABLE registrations ADD COLUMN confirmation_id VARCHAR(20) UNIQUE');
    
    // Add an index for faster lookups
    console.log('Creating index for confirmation_id...');
    await client.query('CREATE INDEX idx_registrations_confirmation_id ON registrations(confirmation_id)');
    
    // Commit transaction
    await client.query('COMMIT');
    
    console.log('Migration completed successfully!');
    console.log('confirmation_id column is now VARCHAR(20) and ready for GRAD format.');
