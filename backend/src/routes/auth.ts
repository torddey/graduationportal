...
router.post('/request-otp', async (req, res) => {
  try {
    ...
    if (!isNaN(studentIdInt)) {
      userRes = await db.query('SELECT student_id AS id, name, email FROM students WHERE student_id = $1', [studentIdInt]);
      identifier = studentIdInt;
    } else {
      userRes = await db.query('SELECT id, username AS name, email FROM admin_users WHERE username = $1 OR email = $1', [userId]);
    ...
    if (role === 'student') {
      ...
      await db.query('DELETE FROM admin_otps WHERE admin_id = $1', [identifier]);
      await db.query(
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    if (!isNaN(studentIdInt)) {
      userRes = await db.query('SELECT student_id, name, email FROM students WHERE student_id = $1', [studentIdInt]);
    ...
    if (!userRes || userRes.rows.length === 0) return res.status(404).json({ error: 'User not found' });
    ...
    if (role === 'student') {
      ...
      if (otpRes.rows.length > 0) {
        await db.query(`UPDATE admin_otps SET used = true WHERE admin_id = $1 AND otp_code = $2`, [identifier, otp]);
      }
    }

    if (!otpRes || otpRes.rows.length === 0) return res.status(400).json({ error: 'Invalid OTP' });
    ...
    const token = jwt.sign(
      { id: finalUserId, name: user.name, role, ...(studentId && { studentId }) },
      JWT_SECRET,
      { expiresIn: '24h' }
    );
