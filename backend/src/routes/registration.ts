...
router.get('/export/:studentId', async (req, res) => {
  try {
    ...
    const ceremonyLocation = settings.ceremony_location || 'GIMPA Main Campus Auditorium';
    const gownCollectionDeadline = settings.gown_collection_deadline || '2025-05-10T14:00:00';
    const gownReturnDeadline = settings.gown_return_deadline || '2025-08-08T23:59:59';
    ...
    doc.fontSize(12)
       .font('Helvetica')
       .fill('#718096')
       .text(`Registration Date: ${new Date(student.registration_date).toLocaleDateString()}`, 50, 190);
    ...
    yPosition += 50;
    const contactInfo = [
      { label: 'Address:', value: formData.address || student.address || 'Not provided' },
      { label: 'City:', value: formData.city || student.city || 'Not provided' },
      { label: 'Postal Code:', value: formData.postalCode || student.postalCode || 'Not provided' },
      { label: 'Country:', value: formData.country || student.country || 'Not provided' }
    ];
    ...
    if (formData.emergencyContact) {
      ...
      yPosition += 50;
      const emergencyInfo = [
        { label: 'Name:', value: formData.emergencyContact.name || 'Not provided' },
        { label: 'Relationship:', value: formData.emergencyContact.relationship || 'Not provided' },
        { label: 'Phone:', value: formData.emergencyContact.phone || 'Not provided' }
      ];
    ...
    doc.fontSize(10)
       .font('Helvetica')
       .fill('#718096')
       .text(`Page 1 of 2 - Generated on: ${new Date().toLocaleDateString()}`, 50, 750, { align: 'center' });
    ...
    const guestInfo = [
      { label: 'Number of Guests:', value: formData.guestCount || 'Not specified' },
      { label: 'Dignitaries Attending:', value: formData.dignitaries === 'yes' ? 'Yes' : 'No' }
    ];
    ...
    doc.fontSize(10)
       .font('Helvetica')
       .fill('#718096')
       .text(`Page 2 of 2 - GIMPA Graduation Portal`, 50, 750, { align: 'center' });
