        ...
        if (response.ok) {
          ...
          setSettings((prev) => ({
            ...
            registrationDeadline:
              data.registrationDeadline?.replace("Z", "") ||
              prev.registrationDeadline,
            ceremonyDate:
              data.ceremonyDate?.replace("Z", "") || prev.ceremonyDate,
            ceremonyLocation: data.ceremonyLocation || prev.ceremonyLocation,
            gownCollectionDeadline:
              data.gownCollectionDeadline?.replace("Z", "") ||
              prev.gownCollectionDeadline,
            gownReturnDeadline:
              data.gownReturnDeadline?.replace("Z", "") ||
              prev.gownReturnDeadline,
          }));
        } else {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Configure system settings for the graduation portal
        </p>
      </div>

      <div className="flex flex-col items-center w-full gap-12">
        <form onSubmit={handleSave} className="max-w-4xl w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Generate this in your Google Account settings under Security
                    ...
                    <div className="ml-3">
                      ...
                      <div className="mt-2 text-sm text-blue-700">
                        <ol className="list-decimal list-inside space-y-1">
                          ...
                          <li>
                            Generate an App Password: Google Account → Security →
                            2-Step Verification → App passwords
                          </li>
                          <li>Use your Gmail address as the username</li>
                          <li>
                            Use the 16-character app password (not your regular
                            password)
                          </li>
                        </ol>
                      </div>
        ...
        {/* Create New Admin Section - only for superadmin */}
        {user?.role === "superadmin" && (
          <div className="bg-white rounded-lg shadow-md overflow-hidden max-w-4xl w-full">
            <div className="bg-gray-50 px-6 py-4 border-b">
              ...
              <p className="text-gray-600 text-sm mt-1">Add a new admin user to the system. Only superadmins can do this.</p>
            </div>
            <CreateAdminForm />
          </div>
        )}

        {/* Only show admin list if user is superadmin */}
        {user?.role === "superadmin" && <AdminList />}
      </div>
    </div>
  );
};
...
const AdminList = () => {
  ...
  if (loading) return <div className="mt-8">Loading admins...</div>;
  if (error) return <div className="mt-8 text-red-600">{error}</div>;
  if (!admins.length) return <div className="mt-8">No admins found.</div>;

  return (
    <div className="bg-white rounded-lg shadow-md overflow-x-auto max-w-4xl w-full mt-12">
      <div className="bg-gray-50 px-6 py-4 border-b">
        <h2 className="text-xl font-semibold text-gray-800">All Admins</h2>
      </div>
